...
export const checkLogged = () => async (dispatch) => {
	try {
		...
		const tokenRes = await Axios.post(
			'http://localhost:5555/api/validToken', 
...
export const emailVerificationSend = (email) => async(dispatch) => {
	try {
		...
		await Axios.post('http://localhost:5555/api/verificationSend', email, config)
...
export const emailVerification = (code) => async(dispatch) => {
	try {
		...
		await Axios.post(
			'http://localhost:5555/api/verificationRight', 
			data, config
		)
...
export const login = (emailLogin, passwordLogin) => async(dispatch) => {
	try {
		...
		const loginRes = await Axios.post('http://localhost:5555/api/login', loginUser, config)
...
export const register = (email,password,confirmPassword,name) => async(dispatch) => {
	try {
		...
		await Axios.post('http://localhost:5555/api/register', newUser, config)
